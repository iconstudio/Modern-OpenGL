...
namespace fpng
{
	...
	bool fpng_encode_image_to_memory(const void* pImage, size_t w, size_t h, size_t num_chans, std::vector<uint8_t>& out_buf, uint32_t flags = 0);
	...
	int fpng_get_info(const void* pImage, size_t image_size, size_t& width, size_t& height, size_t& channels_in_file);
	...
	// If FPNG_DECODE_NOT_FPNG is returned, you must decompress the file with a general purpose PNG decoder.
	// If another error occurs, the file is likely corrupted or invalid, but you can still try to decompress the file with another decoder (which will likely fail).
	int fpng_decode_memory(const void* pImage, size_t image_size, std::vector<uint8_t>& out, size_t& width, size_t& height, size_t& channels_in_file, size_t desired_channels);

#ifndef FPNG_NO_STDIO
	int fpng_decode_file(const char* pFilename, std::vector<uint8_t>& out, size_t& width, size_t& height, size_t& channels_in_file, size_t desired_channels);
#endif
